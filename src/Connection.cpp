 ...
 */
void Connection::installModules(const std::vector<ModuleInstallation>& modules,
                                ...
                                const libyang::DataFormat dataFormat)
{
    ...
    /* prepare a string of search directories in <dir>[:<dir>]* format for sysrepo,
       FIXME - change to support Windows */
    ...
    auto res = sr_install_modules2(ctx.get(),
                                   mods.data(),
                                   mods.size(),
                                   searchDirsStr.empty() ? nullptr : searchDirsStr.c_str(),
                                   initData ? initData->c_str() : nullptr,
                                   initPath ? initPath->c_str() : nullptr,
                                   static_cast<LYD_FORMAT>(dataFormat));
 ...
 */
void Connection::removeModules(const std::vector<std::string>& modules, const sysrepo::ModuleRemoval opt)
{
    auto modulesC = unsafeCStringArray(modules);
    auto res = sr_remove_modules(ctx.get(), modulesC.data(), opt == sysrepo::ModuleRemoval::FailOnDependencies ? 1 : 0);
