                                ...
                                const libyang::DataFormat dataFormat)
{
    ...
    for (std::size_t i = 0; i < modules.size(); ++i) {
        /*
         * Value-initialize the whole struct (all bytes zeroed, *including* the trailing padding).
         * In sr_install_modules2() sysrepo does `memcpy(&new_mods[i], &modules[i], sizeof *modules)`
         * (src/sysrepo.c:1842, sysrepo commit e92dd4fc) into an sr_int_install_mod_t (src/common.h),
         * whose `int is_schema_yang` member overlaps exactly the tail padding of sr_install_mod_t.
         * A designated-initializer would leave that padding indeterminate; a non-zero value there
         * makes sysrepo interpret schema_path as inline YANG data and fail with
         * "Invalid YANG module data".
         */
        auto& mod = mods.emplace_back();
        mod.schema_path = allSchemas[i].c_str();
        mod.features = allFeatures[i].data();
        mod.owner = modules[i].owner ? modules[i].owner->c_str() : nullptr;
        mod.group = modules[i].group ? modules[i].group->c_str() : nullptr;
        mod.perm = modules[i].permissions;
    }
