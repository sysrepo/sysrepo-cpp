        ...
        const libyang::DataFormat dataFormat)
{
    ...
    auto res = sr_install_modules2(ctx.get(), mods.data(), mods.size(),
            searchDirs ? searchDirs->c_str() : nullptr, initData ? initData->c_str() : nullptr,
            initPath ? initPath->c_str() : nullptr, static_cast<LYD_FORMAT>(dataFormat));
