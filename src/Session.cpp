...
libyang::DataNode wrapSrData(std::shared_ptr<sr_session_ctx_s> sess, sr_data_t* data)
{
    return libyang::wrapRawNode(data->tree, std::shared_ptr<sr_data_t>(data, [sess] (sr_data_s* data) {
        // The data inside sr_data_t can be modified. The user could for example completely change the tree. This means
        // that the `tree` variable inside the sr_data_t struct might point to some place that the user already freed
        // (for example via `unlink()`). Because of this, we're taking the lifetime of this tree into our own hands with
        // our own memory management (note that we're using wrapRawNode, not wrapRawUnmanagedNode!).
        //
        // Because we're freeing the tree by ourselves, we need to set data->tree to nullptr, to prevent sr_release_data
        // from feeing it.
        data->tree = nullptr;
