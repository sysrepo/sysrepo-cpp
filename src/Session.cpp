...
YangPushSubscription Session::yangPushPeriodic(sysrepo::Datastore datastore, std::chrono::milliseconds periodTime, const std::optional<std::string>& xpathFilter, const std::optional<NotificationTimeStamp>& anchorTime, const std::optional<NotificationTimeStamp>& stopTime)
{
    int fd;
    uint32_t subId;
...
YangPushSubscription Session::yangPushOnChange(sysrepo::Datastore datastore, const std::optional<std::string>& xpathFilter, bool syncOnStart, const std::chrono::milliseconds& dampeningPeriod, const std::optional<NotificationTimeStamp>& stopTime)
{
    int fd;
    uint32_t subId;
    auto stopSpec = stopTime ? std::optional{toTimespec(*stopTime)} : std::nullopt;
