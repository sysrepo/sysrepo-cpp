...
struct CustomEventLoopCallbacks {
     ...
     * This function is supposed to register polling of file descriptor `fd`. The processEvents callback should be saved
     * and later called when reading is available on the file descriptor.
     */
    std::function<void(int fd, std::function<void()> processEvents)> m_registration;
    /*
     ...
     */
    std::function<void(int fd)> m_unregistration;
};
