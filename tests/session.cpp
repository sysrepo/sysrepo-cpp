...
TEST_CASE("session")
{
  ...
  "test_module:popelnice": {
    ...
    DOCTEST_SUBCASE("libyang context flags")
    {
        ...
        {
            conn->setGlobalContextOptions(sysrepo::ContextFlags::LibYangPrivParsed | sysrepo::ContextFlags::NoPrinted);
        ...
        DOCTEST_SUBCASE("sysrepo::setGlobalContextOptions LibYangPrivParsed")
        {
            sysrepo::setGlobalContextOptions(sysrepo::ContextFlags::LibYangPrivParsed | sysrepo::ContextFlags::NoPrinted);
            // ...but that call is not neccessarily propagated to all the existing connections,
            // so we have to create a new one to be sure...
            auto sess2 = sysrepo::Connection{}.sessionStart();
