...
#define REQUIRE_YANG_PUSH_ONCHANGE(SUBSCRIPTION, JSON_DATA)                                                    \
    ...
    REQUIRE(pipeHasData(SUBSCRIPTION->fd()));                                                                  \
...
TEST_CASE("Dynamic subscriptions")
{
  ...
  "ietf-subscribed-notifications:subscription-terminated": {
    ...
    "reason": "no-such-subscription"
        ...
        {
            auto sub = std::make_unique<sysrepo::DynamicSubscription>(sess.subscribeNotifications("/ietf-subscribed-notifications:subscription-terminated", std::nullopt, stopTime));
    ...
    "reason": "no-such-subscription"
        ...
        while (true) {
            ...
            if (r == -1) {
                ...
                FAIL("poll() timed out");
            } else if (r == 1 && pfds[0].revents & POLLIN) {
                REQUIRE(pipeHasData(sub.fd()));
                sub.processEvent(cb);
            } else if (r == 1 && pfds[0].revents & POLLHUP) {
                break;
            } else {
                FAIL("poll() unexpected return value");
            }
        }
