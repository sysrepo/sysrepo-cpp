...
TEST_CASE("Yang push subscriptions")
{
  ...
  "ietf-subscribed-notifications:subscription-terminated": {
    ...
    {
        auto sub = sess.subscribeNotifications(std::nullopt);
        std::this_thread::sleep_for(50ms);

        TROMPELOEIL_REQUIRE_CALL(rec, recordNotification(R"({
  "test_module:ping": {
    "myLeaf": 1
  }
}
        ...
        {
            auto notification = client.getContext().newPath("/test_module:ping");
            notification.newPath("myLeaf", "1");
            client.sendNotification(notification, sysrepo::Wait::No);
        }

        REQUIRE(pipeHasData(sub.fd()));
        sub.processEvent(cbNotif);
