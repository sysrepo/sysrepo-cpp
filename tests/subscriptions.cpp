...
TEST_CASE("subscriptions")
{
    ...
    DOCTEST_SUBCASE("Moving items")
    {
        ...
        sess.applyChanges();
        Recorder rec;
        sysrepo::ModuleChangeCb moduleChangeCb = [&rec] (sysrepo::Session session, auto, auto, auto, auto event, auto) -> sysrepo::ErrorCode {
            // We must react to the "Change", because trompeloeil throws, and throwing in "Done" events does nothing
        ...
        auto sub = sess.onModuleChange("test_module", moduleChangeCb, nullptr, 0);
    ...
    DOCTEST_SUBCASE("notifications")
    {
        ...
        sysrepo::NotifCb cb = [&rec] (auto, auto, auto type, auto notification, auto) {
            rec.recordNotification(type, notification ? std::optional{std::string{notification->path()}} : std::nullopt);
        };
    ...
    {
        sysrepo::ModuleChangeCb moduleChangeCb = [fail = true] (auto session, auto, auto, auto, auto, auto) mutable -> sysrepo::ErrorCode {
            if (fail) {
                session.setErrorMessage("Test error message.");
                fail = false;
        ...
        try {
            ...
            REQUIRE(*errors.at(0).errorMessage == "Test error message.");
