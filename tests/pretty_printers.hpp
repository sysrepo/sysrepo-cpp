...
namespace sysrepo {
doctest::String toString(const ErrorInfo& info)
{
    std::ostringstream oss;
    oss << "sysrepo::ErrorInfo {\n";
    oss << "    code: " << static_cast<std::underlying_type_t<decltype(info.code)>>(info.code) << "\n";
    oss << "    message: " << info.errorMessage << "\n";
    oss << "\n}";
    return oss.str().c_str();
}
