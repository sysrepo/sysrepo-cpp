...
TEST_CASE("Yang push subscriptions")
{
    ...
    {
        auto sub = std::make_unique<sysrepo::YangPushSubscription>(sess.yangPushOnChange(std::nullopt, sysrepo::SyncOnStart::Yes));
  ...
  "ietf-subscribed-notifications:subscription-terminated": {
    "reason": "no-such-subscription"
        ...
        {
            sub->terminate();
            REQUIRE_THROWS_WITH_AS(sub->terminate(),
                                   ("Couldn't terminate yang-push subscription with id " + std::to_string(sub->subscriptionId()) + ": SR_ERR_NOT_FOUND").c_str(),
    ...
    {
        auto sub = sess.yangPushPeriodic(std::chrono::milliseconds{100}, std::nullopt, std::nullopt, std::chrono::system_clock::now() + 1333ms);
